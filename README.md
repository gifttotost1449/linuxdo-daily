...
推荐方式：使用 `LINUXDO_ACCOUNTS`，每个账号一行或使用 `;` 分隔，格式为 `username:password`。

示例：
```
LINUXDO_ACCOUNTS="user1:pass1;user2:pass2"
```
    ...
    - 安装Python依赖
      - 进入青龙面板 -> 依赖管理 -> 安装依赖
        ...
        - 名称填写(仓库`requirements.txt`文件的完整内容)：
            ...
            ```
        - 点击确定
      - 若脚本开启 `AUTO_INSTALL_DEPS` 且运行环境有 pip，可自动安装依赖；没有 pip 仍需按上述步骤配置
